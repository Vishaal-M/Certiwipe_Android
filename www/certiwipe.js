...
function startRustWipe(method, drive) {
  ...
  // Determine Rust executable path for development or packaged build
  let rustExe;
  if (process.env.NODE_ENV === 'development') {
    rustExe = path.join(__dirname, 'rust', 'certiwipe.exe'); // dev folder
  } else {
    rustExe = path.join(process.resourcesPath, 'rust', 'certiwipe.exe'); // packaged
  }

  // Spawn Rust executable
